...
export class TokenService {
    constructor(private readonly TOKEN_SECRET: string, private readonly TOKEN_EXPIRES_IN: string) {
    ...
    decode(token: string): Result<JwtPayload, Failure<TokenErrorsEnum.INVALID_JWT>> {
        if (!isValidJwt(token)) {
    ...
    verify(token: string): Result<JwtPayload, Failure<TokenErrorsEnum.INVALID_JWT | TokenErrorsEnum.TOKEN_EXPIRED>> {
        if (!isValidJwt(token)) {
            return err(TokenErrorFactory(TokenErrorsEnum.INVALID_JWT));
        }
        try {
