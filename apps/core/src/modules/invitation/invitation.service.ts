...
import {
  InvitationErrorFactory,
  InvitationErrorsEnum,
} from './invitation.exceptions';
...
export class InvitationService {
  ...
  async fetchInvitationUsingId(
    ...
    Result<
      InvitationDocument,
      Failure<InvitationErrorsEnum.INVITATION_NOT_FOUND>
    >
  > {
    const invitation:
      | InvitationDocument
      | undefined = await this.invitationModel.findOne({ _id: id });
    if (isTruthy(invitation)) {
      return ok(invitation);
    } else {
      return err(
        InvitationErrorFactory(InvitationErrorsEnum.INVITATION_NOT_FOUND),
      );
    }
  }
  ...
  async newInvitation(
    ...
    Result<
      InvitationDocument,
      Failure<InvitationErrorsEnum.ERROR_CREATING_INVITATION>
    >
  > {
    try {
      const invitation: InvitationDocument = await this.invitationModel.create({
        creatorOfInvitation,
      });
      return ok(invitation);
    } catch (e) {
      return err(
        InvitationErrorFactory(InvitationErrorsEnum.ERROR_CREATING_INVITATION),
      );
    }
  }
  ...
  async invitationOpened(
    ...
    Result<
      ...
      Failure<
        ...
        | InvitationErrorsEnum.FORBIDDEN_TO_OPEN_INVITATION
        | ChatErrorsEnum.ERROR_CREATING_CHAT
        | ChatErrorsEnum.CHAT_ALREADY_EXIST
      >
    >
  > {
    ...
    const chatFindResult = await this.chatService.findChat(
      invitationId,
      openedBy.id,
    );
